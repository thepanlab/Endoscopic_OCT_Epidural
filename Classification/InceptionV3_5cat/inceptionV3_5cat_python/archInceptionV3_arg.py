...
for index in a_selected_epidural_test:
    ...
    a_images_1D_7_epidurals = a_images_1D[bool_epidural_num]
    ...
    for index_val in a_epidural_num_val:
        ...
        model_inception = keras.models.Model(inputs=base_model_empty.input, outputs=output)
        ...
        model_inception.compile(loss="sparse_categorical_crossentropy", optimizer=optimizer,
                      metrics=["accuracy"])
        ...
        model_inception.save("/gpfs/alpine/bif121/proj-shared/epidural/epidural_results/archInceptionV3_5cat_results/model_outer_K%s_outer_k%s_val.h5"%(index, index_val))

        print( "Elapsed time during the whole program in seconds for K%s_outer_k%s_val: "%(index, index_val), time_lapse)
        
        with open('/gpfs/alpine/bif121/proj-shared/epidural/epidural_results/archInceptionV3_5cat_results/time_total_K%s_outer_k%s_val.npy'%(index, index_val), 'wb') as f:
        ...
        y_proba = model_inception.predict(X_val)
                
        with open('/gpfs/alpine/bif121/proj-shared/epidural/epidural_results/archInceptionV3_5cat_results/pred_val_K%s_outer_k%s_val.npy'%(index, index_val), 'wb') as f:
            np.save(f, y_proba)
        
        with open('/gpfs/alpine/bif121/proj-shared/epidural/epidural_results/archInceptionV3_5cat_results/history_K%s_outer_k%s_val.pickle'%(index, index_val), 'wb') as handle:
