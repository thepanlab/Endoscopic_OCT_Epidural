...
for index in a_selected_epidural_test:
    ...
    model_resnet50.save("/gpfs/alpine/bif121/proj-shared/epidural/epidural_results/archRESNET50_bin4_test_results/model_outer_K%s.h5"%(index))
    ...
    with open('/gpfs/alpine/bif121/proj-shared/epidural/epidural_results/archRESNET50_bin4_test_results/time_total_K%s_outer.npy'%(index), 'wb') as f:
        np.save(f, np.array(time_lapse))

    y_proba = model_resnet50.predict(X_test)
            
    with open('/gpfs/alpine/bif121/proj-shared/epidural/epidural_results/archRESNET50_bin4_test_results/pred_val_K%s_outer.npy'%(index), 'wb') as f:
        np.save(f, y_proba)
    
    with open('/gpfs/alpine/bif121/proj-shared/epidural/epidural_results/archRESNET50_bin4_test_results/history_K%s_outer.pickle'%(index), 'wb') as handle:
